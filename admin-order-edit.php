...
$data = $orderCollection->aggregate([['$match' => ['_id' => new ObjectId($_GET['id'])]], [
    '$lookup' =>
    [
        'from' => "order_detail",
        'localField' => "_id",
...
if (isset($_POST["submit"])) {
    ...
    if (empty($_POST['status'])) {
        $errorValidation['status'] = "Trạng thái đơn hàng không được để trống";
    } elseif (!empty($_POST['status']) && !in_array($_POST['status'], [ORDER_PHASE_1, ORDER_PHASE_2, ORDER_PHASE_3, ORDER_PHASE_4, ORDER_PHASE_5])) {
        $errorValidation['status'] = "Trạng thái đơn hàng không đúng định dạng";
    }

    // Check validate
    if (count($errorValidation) == 0) {
        $updateResult = $orderCollection->updateOne(["_id" => $data[0]->_id], [
            '$set' => [
                'status'    => $_POST['status'],
            ]
        ]);
                            ...
                            <div class="row box-content-dashboard" style="width: 90%">
                                <table>
                                    ...
                                    <?php foreach ($data[0]->data_order_detail as $orderDetail) : ?>
                                        ...
                                        <tr>
                                            <td>
                                                <a href="./admin-product-edit.php?id=<?= !empty($dataProduct->_id) ? $dataProduct->_id : '' ?>">
                            ...
                            <div class="content-box-form content-box-login">
                                <form action="" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="btn-form btn-login">
                                        <button class="btn" type="submit" name="submit">Cập nhật trạng thái đơn hàng</button>
                                        <a href="./admin-order.php" class="btn">Trở lại</a>
                                    </div>
                                </form>
